...
const SearchExercises = ({setExercises , bodyPart , setBodyPart}) => {
  ...
  //we'll use ueEffect cz we want to show a list of body parts even before searching
  useEffect(() => {
  ...
  }, []) //dependency array is going to be empty because we only want ot call the useEffect in the beginning
  ...
  const handleSearch = async () => {
    if(search) {
      ...
      setSearch('') //once done with the search we are making it to be an empty string again
      setExercises(searchedExercises) //putting the retrieved exercises in the exercises state
    }
  }
